...
attr(beta.model, "backsolve") = function(theta, y, log=FALSE)
{
  ...
  x0 = delta1*delta3/(delta1+delta2)
...
attr(beta.model, "gradient") = function(theta, x)
{
  ...
  index = x>0
  temp = exp(.beta.func(delta1, delta2, log=TRUE) + delta1*log(x[index]) + delta2*log(delta3-x[index]) - (delta1+delta2)*log(delta3) )
...
attr(beta.model, "start") = function(x, y)
{
    beta0 = rep(0, 5)
